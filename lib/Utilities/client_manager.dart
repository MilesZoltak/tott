...
class ClientManager {
  ...
  Future uploadSignature(String firstName, String lastName,
      String imageData) async {
    const baseUrl = "https://webservices.sagebridge.org/";
    const route = "/v3/subpopulations/{subpopulationGuid}/consents/signature";
    final url = Uri.parse(baseUrl + route);

    String sessionToken = await SecureStorageManager().getSessionToken();

    final data = {
      "subpopulationGuid": "tott-sandbox",
      "ConsentSignature": {
  ...
  Future<ParticipantData?> checkExistingOnboardingInfo(String email,
    ...
    try {
      ...
      if (response.statusCode == 401) {
        ...
        ParticipantData? existingParticipant = json.decode(response.body);
        print(
            "Successfully checked for existing onboarding data.  Data is ${existingParticipant ==
                null ? "null" : "not null"}.");
