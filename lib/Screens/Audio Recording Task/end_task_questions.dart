...
class EndTaskQuestions extends StatefulWidget {
  ...
  EndTaskQuestions(this.taskList, this.participant, {super.key});
...
class _EndTaskQuestionsState extends State<EndTaskQuestions> {
    ...
    PageViewModel whichChildPage = PageViewModel(
        ...
        bodyWidget: ListView.separated(
            ...
            itemBuilder: (context, index) {
              String name = widget.participant[index].nickname ?? "[NO NAME]";
              return CheckboxListTile(
                  value: participatingChildren.contains(name),
                  onChanged: (val) {
                    if (val == null) return;

                    if (val) {
                      participatingChildren.add(name);
                    } else {
                      participatingChildren.remove(name);
                    }
                  });
            },
  ...
  Future manageTaskList() async {
    ...
    if (widget.participant.length != 1) {
